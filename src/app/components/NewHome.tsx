...
import { mockGalleryItems, mockAgents, mockTransactions } from '../data/mockData';

export function NewHome() {
  const navigate = useNavigate();
  const { data: metrics } = useMetrics(8000);
  const { data: apiTx } = useTransactions(5000);
  const { data: apiGallery } = useGallery(undefined, 8000);
  const { data: apiAgents } = useAgents(8000);

  const transactions = apiTx ?? mockTransactions;
  const galleryItems = apiGallery?.items ?? mockGalleryItems;
          ...
          {featured.map(item => (
            ...
            >
              ...
              <div className="flex items-center justify-between mt-0.5">
                ...
                <span className="text-sm font-semibold" style={{ color: 'var(--g-gold)' }}>{item.price} cr</span>
              </div>
            </motion.div>
          ))}
        </div>
